...
Token* readIdentKeyword(void) {
  	...
  	// neu la chu hoac so thi tiep tuc doc.
  	while (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar]){
  		token->string[index] = (char)currentChar;
  		index++;
  		readChar();
	  }
	token->string[index] = '\0';
	// kiem tra tu co vuot qua do dai ?
	if (index > MAX_IDENT_LEN){
		error(ERR_IDENTTOOLONG, lineNo, colNo);
	} else {
		// kiem tra co phai la tu khoa khong ?
